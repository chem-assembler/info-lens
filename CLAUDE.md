...
- **参照は `?v=N` を付け、中身を変えたら N を上げる**（2026-08-12。同日中に3回踏んで確定した規則）。
  GitHub Pages のキャッシュは **HTML が 600 秒、それ以外のアセットは 14400 秒（4時間）**。
  つまり:
  - **新規ファイル**: 公開前にその URL を叩くと、404 が4時間キャッシュされ、実体を置いても
    404 が返り続ける（HTML だけ先に新しくなり「読めないファイルを参照する HTML」になる）。
    → `?v=1` を付けた新しい URL にして避ける
  - **更新**: `?v=` を据え置くと、ブラウザとCDNが**古い実体を4時間使い続ける**。
    js/css を直したのに画面が変わらないのはこれ（2026-08-12、digitize の M2 で実際に発生）。
    → **中身を変えたら必ず N を上げる**
  - **確認**: 参照している URL そのもの（`?v=1`）を叩かない。`?nc=<乱数>` のような
    使い捨てのクエリで見る。掴んでしまったら参照側の N を上げる
  - **素の参照（`?v=` 無し）を残さない**。上げるべき N が無いので、この規則がまるごと空振りする
    （2026-09-12、dncl の9件中7件がこれだった）。**参照を新しく足すときは最初から `?v=1`**
  アプリ全体で番号をそろえる必要はない（ファイル単位でよい）。chem のような一括版番号は入れない

...
6. **ファイルを新しく足したら、参照側に `?v=1` を付ける。**
   GitHub Pages は 404 を4時間キャッシュするので、公開前にその URL を叩いてしまうと、
   ファイルを置いた後もしばらく 404 が返り続ける（＝新しい HTML が読めないファイルを参照する）。
   既存ファイルの更新に版番号は要らない（10分で入れ替わる）。
   公開できたかの確認は `?nc=<乱数>` のような別のクエリで行う（参照している URL を叩いて
   404 を掴むと、その URL が4時間死ぬ）。

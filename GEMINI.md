...
6. **参照は `?v=N`。中身を変えたら N を上げる。**
   GitHub Pages のキャッシュは HTML が 600 秒、js/css/画像は **4時間**。
   `?v=` を据え置いたままファイルを直すと、古い実体が4時間使われ続ける（画面が変わらない）。
   新しく足したファイルに `?v=1` を付けるのは、公開前にその URL を叩いて 404 を
   掴んでしまう事故（404 も4時間キャッシュされる）を避けるため。
   公開できたかの確認は `?nc=<乱数>` のような使い捨てのクエリで行う。
   **素の参照（`?v=` 無し）を残さない。** 上げるべき N が無いと、この規則がまるごと空振りする
   （2026-09-12、dncl の9件中7件がこれだった）。**参照を新しく足すときは最初から `?v=1` を付ける。**
7. **録画モード（`dncl/rec.js`）は `?rec=` が無ければ完全に不活性であること。**

const problems = [
  ...
  },
  {
    id: "leap_year",
    title: "うるう年の判定",
    description: "変数 西暦（例: 2024）が「うるう年」であるか判定するプログラムを完成させてください。うるう年の場合は「1」、そうでない場合は「0」を表示します。（4で割り切れ、かつ100で割り切れない、または400で割り切れる年がうるう年です）",
    difficulty: "中級",
    initialState: {
      西暦: 2024,
      うるう年: 0
    ...
    explanation: "「うるう年 = 0」で非うるう年を初期状態とします。4の倍数の場合に最初の条件に入り、さらに「100の倍数ではない、または400の倍数である」という条件をネスト（入れ子）で判定し、適合すれば「うるう年 = 1」に変更して最後に表示します。【入れ替え可能な箇所】初期化「西暦 = 2024」と「うるう年 = 0」は順序を入れ替えても動作します。"
  },
  {
    id: "find_max",
    title: "配列の最大値を見つける",
  ...
  },
  {
    ...
    description: "変数 A（48）と B（18）の最大公約数（GCD）を、ユークリッドの互除法を使って計算し表示するプログラムを完成させてください。Bが0になるまで、AをBで割った余りを求め続けます。",
    difficulty: "中級",
    initialState: {
      A: 48,
      B: 18
    ...
    correctBlocks: [
      { id: "b1", text: "A = 48", indent: 0 },
      { id: "b2", text: "B = 18", indent: 0 },
    ...
    explanation: "ユークリッドの互除法では、「AをBで割った余りを求め、AにB、Bに余りを代入する」という操作を、余りが0（すなわち、割る数Bが0）になるまで繰り返します。ループ終了時の「A」に最大公約数が残っています。【入れ替え可能な箇所】初期化「A = 48」と「B = 18」は順序を入れ替えても動作します。"
  },
  {
    id: "linear_search",
    title: "線形探索（サーチ）",
  ...
  },
  {
    id: "coin_change",
    title: "最小硬貨枚数（お釣り計算）",
    description: "お釣り金額 780 円を、500円、100円、50円、10円硬貨を使って、最小の枚数で支払う際、各硬貨の枚数を計算し表示するプログラムを完成させてください。（DNCLでは「商の整数部分」を取り出す際に「の整数部分」を使います）",
    difficulty: "上級",
    initialState: {
      金額: 780,
      硬貨: [500, 100, 50, 10],
...
const syntaxLessons = [
  ...
  },
  {
    id: "lesson_branch",
    title: "2. 条件分岐（もし〜ならば）",
    description: "点数（得点）が80点以上なら「合格」、そうでなければ「不合格」と表示するプログラムを完成させてください。",
    initialState: {
      得点: 85
    },
    correctBlocks: [
